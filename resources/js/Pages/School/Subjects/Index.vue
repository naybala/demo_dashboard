...
<template>
  <Head>
    <title>{{ __("school.subjects", "Subjects") }}</title>
  </Head>
  ...
  <AdminLayout>
    ...
    <div class="py-6">
      <div class="mx-auto sm:px-6 lg:px-8">
        ...
        >
          <div>
            ...
            <p class="text-sm text-gray-500 dark:text-gray-400 font-medium">
              {{
                __(
                  "school.subjects_subtitle",
                  "Manage school subjects and curricula.",
                )
              }}
            </p>
          </div>
          <div class="flex items-center gap-3">
            ...
            >
              ...
              {{ __("messages.create", "Create Subject") }}
            </PrimaryButton>
          </div>
        </div>
          ...
          <div class="flex flex-col md:flex-row gap-4 items-end">
            <div class="flex-1 w-full">
              <TextInput
                type="text"
                v-model="search"
                :placeholder="__('messages.search_item', 'Search subjects...')"
        ...
        >
          ...
          >
            Showing {{ data.length }} of {{ meta.total }} subjects
          </div>
          ...
          <BaseTable :headers="headers">
            ...
            >
              ...
              <td class="px-6 py-4 text-sm text-center">
                <div class="flex items-center justify-center gap-2">
                  <SecondaryButton
                    v-if="permissions.includes('edit subjects')"
                    @click="openEditModal(subject)"
                  >
                    {{ __("messages.edit", "Edit") }}
                  </SecondaryButton>
                  <SecondaryButton
                    v-if="permissions.includes('delete subjects')"
                    variant="danger"
            ...
            <tr v-if="data.length === 0">
              <td colspan="4" class="px-6 py-10 text-center text-gray-500">
                {{ __("messages.no_data", "No subjects found.") }}
              </td>
            </tr>
          </BaseTable>
    ...
    <DeleteConfirmationModal
      ...
      :title="__('school.delete_subject_title', 'Delete Subject')"
      :message="
        __(
          'school.delete_subject_message',
