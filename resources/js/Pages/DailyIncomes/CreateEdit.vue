...
} = useDailyIncomeForm(props.dailyIncome, props.products);

const totalAmount = computed(() => formatNumber(totalAmountValue.value, 0));
</script>
    ...
    <div class="bg-white dark:bg-gray-800 shadow-sm sm:rounded-lg p-6">
      <form @submit.prevent="submit" class="space-y-6">
          ...
          <div class="flex items-end">
            <label class="inline-flex items-center mb-2">
              ...
              <span class="ms-2 text-sm text-gray-600 dark:text-gray-400">
                {{ __("dailyIncome.is_instant", "Is Instant Payment") }}
              </span>
            </label>
          </div>
        </div>
              ...
              <table class="w-full text-sm text-left">
                ...
                <tbody class="divide-y divide-gray-200 dark:divide-gray-600">
                  <tr v-for="(item, i) in form.items" :key="i">
                    <td class="px-4 py-2">
                      <SearchableSelect
                        async-url="/own-products/search"
                        :options="productOptions"
                        v-model="item.own_product_id"
                        @change="(opt) => handleProductChange(i, opt.original)"
                        :placeholder="
                          __('placeholder.select_product', 'Select Product')
                        "
                      />
                    </td>
                    <td class="px-4 py-2">
        ...
        <div class="flex items-center justify-end gap-4">
          <SecondaryButton @click="router.get('/daily-incomes')">
            {{ __("messages.cancel", "Cancel") }}
          </SecondaryButton>
          <template
            v-if="
              (props.dailyIncome &&
                permissions.includes('edit daily-incomes')) ||
              (!props.dailyIncome &&
                permissions.includes('create daily-incomes'))
            "
          >
