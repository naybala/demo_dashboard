@props(['title','required'=>false,'name','id','selectedUser'=>null])
<x-form.control>
    <x-form.label :title="$title" :required="$required" />

    <div
        x-data="{
            search: '{{ $selectedUser ? $selectedUser['fullname'].'-'.$selectedUser['phone_number'] : ''}}',
            users: [],
            selectedUser: {{ $selectedUser ? json_encode($selectedUser) : 'null' }},
            loading: false,
            isOpen: false,
            timeout: null,
            async fetchUsers() {
